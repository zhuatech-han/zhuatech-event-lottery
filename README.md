...
- 导出活动备份后，可在同一站点换设备恢复管理权限、手动名单、奖项和中奖结果。
...
主办方当前浏览器保存手动名单、奖项、中奖结果和活动管理凭证。创建活动后请导出活动备份，私下保存；备份包含管理凭证，持有者可管理和删除该活动。换设备时可在**同一站点**导入备份并重新同步扫码名单，背景图片须重新上传。活动已删除或过期后无法恢复管理。活动结束后另行导出中奖 CSV，再删除活动。签到链接本身不包含管理凭证，背景图片不上传服务器。
